...
class BaseDetailView(generic.ObjectView):
    ...
    def generate_mermaid_diagram(self, instance, max_depth=1):
        ...
        def add_node(obj, parent_label=None, current_depth=0):
            ...
            display_name = re.sub(r'[^a-zA-Z0-9_\ \/]', '', display_name)  # Replace quotes to avoid breaking Mermaid syntax
        ...
        for obj_type, color in color_map.items():
            legend += f'classDef color_{obj_type} fill:{color},stroke:#fff,stroke-width:0px,font-weight:bold;\n'
            diagram += f'classDef color_{obj_type} fill:{color},stroke:#fff,stroke-width:0px,font-weight:bold;\n'
